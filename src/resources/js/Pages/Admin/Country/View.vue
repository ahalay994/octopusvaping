    ...
    <BreezeAuthenticatedLayout>
        <template #header>
            <div class="d-flex justify-content-between">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight mb-0 d-flex align-items-center">Страны-производители</h2>
                <a class="btn btn-success" href="javascript:void(0)" @click="addItem">Добавить страну</a>
            </div>
        </template>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <BreezeInput v-if="item.state !== 'view' && item" :id="`name-${index}`" type="text"
                                                 class="mt-1 block w-full" v-model="countries[index].name"/>
                                </td>
...
export default {
    ...
    methods: {
        get() {
            axios.get(this.route('api.country.get'))
                .then(response => {
        ...
        editItem(id) {
            this.countries[id].state = 'edit';
            this.state = 'edit';
        },

        deleteItem(id) {
            axios.delete(this.route('admin.country.delete', id))
                .then(response => {
                    this.get();
                })
        ...
        submit() {
            this.form = this.$inertia.form({countries: this.countries}, {
                resetOnSuccess: true,
                forceFormData: true
            });
            this.form.post(this.route('admin.country.save'), {
                onSuccess: (response) => {
